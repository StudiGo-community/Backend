...
class ProfileView(APIView, PermissionClass):
    ...
    @extend_schema(
        ...
        description=
        "사용자가 마이페이지에 접속하여 본인의 기본 프로필 정보를 조회합니다."
        "마이페이지 접근 경로:"
...
프로필 이미지 업로드 POST
"""
class ProfileImageView(APIView, PermissionClass):
...
비밀번호 변경 PUT
"""
class PasswordChangeView(APIView, PermissionClass):
    @extend_schema(
        tags=["마이페이지"],
